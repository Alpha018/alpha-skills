...
2. That workflow completing triggers [`.github/workflows/publish-nextdns-mcp.yml`](../../.github/workflows/publish-nextdns-mcp.yml) via `workflow_run` (a plain tag-push trigger doesn't fire here: pushes made with the default `GITHUB_TOKEN`, which is what semantic-release uses, don't trigger other workflows). It checks out the commit the release workflow left on `main`, skips if that version is already on npm, and otherwise runs `npm publish` directly. No `NPM_TOKEN` secret: the npm CLI (≥ 11.5.1, hence the explicit upgrade step) detects the GitHub Actions OIDC context from `id-token: write` and exchanges it for a short-lived publish token.
