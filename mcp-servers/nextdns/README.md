...
Two shared workflows at the repo root cover every package under `mcp-servers/`, not just this one:

- [`.github/workflows/pr-test-mcp-servers.yml`](../../.github/workflows/pr-test-mcp-servers.yml) runs on every pull request. It detects which `mcp-servers/<package>` directories changed and runs lint, typecheck, build, and test for each, across Node 22 and 24.
- [`.github/workflows/build-publish-mcp-servers.yml`](../../.github/workflows/build-publish-mcp-servers.yml) runs on push to `main`. For each changed package it builds, tests, then runs [semantic-release](https://semantic-release.gitbook.io) (scoped to that package via `semantic-release-monorepo` and this package's `.releaserc.json`, with `npmPublish` disabled since `@semantic-release/npm` only knows how to authenticate with a classic `NPM_TOKEN`, not npm's OIDC trusted publishing). It bumps `package.json`, writes `CHANGELOG.md`, and cuts a GitHub release and matching `nextdns-mcp-v*` tag from the Conventional Commit types on the merged commits (`fix` → patch, `feat` → minor, `BREAKING CHANGE` → major). In the same job, once semantic-release has bumped the version, it compares against what's live on npm and runs `npm publish` directly if they differ. No `NPM_TOKEN` secret: the npm CLI (≥ 11.5.1, hence the explicit upgrade step) detects the GitHub Actions OIDC context from `id-token: write` and exchanges it for a short-lived publish token.

For publishing to work, `@alpha018/nextdns-mcp` needs a one-time [Trusted Publisher](https://docs.npmjs.com/trusted-publishers) link on npmjs.com (package Settings page) pointing at this repo and the `build-publish-mcp-servers.yml` workflow.
